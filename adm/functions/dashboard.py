...
class Dashboard():
    ...
    def sales_per_country_day():
        ...
        for country in countries:
            for key,value in country.items():
                sales = Sale.objects.filter(created_at__range=(str(today.date())+' 00:00:00', str(today.date())+' 23:59:59'), customer__userdetail__country=value)
                if not value:
                    continue
    ...
    def sales_per_country_month():
        ...
        for country in countries:
            for key,value in country.items():
                sales = Sale.objects.filter(created_at__range=(start, end), customer__userdetail__country=value)
                if not value:
                    continue
                sales_country[value] = sales.aggregate(Sum('payment_amount'))
        return sales_country
    ...
    def sales_per_account():
        ...
        acc_name = []
        acc_total = []
        acc = Sale.objects.filter(created_at__range=(start,end)).exclude(payment_amount=0).values('account__account_name').order_by('account__account_name').distinct()
        for a in acc:
            for key,value in a.items():
                sales = Sale.objects.filter(created_at__range=(start,end),account__account_name=value)
